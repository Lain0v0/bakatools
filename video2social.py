...
def convert_to_mp4(input_path):
    ...
    output_path = os.path.join(output_dir, f"{video_name}_social.mp4") # 文件名后缀
    ...
    try:
        ...
        ffmpeg_cmd = [
            'ffmpeg',
            '-i', input_path,
            '-vf', f'scale={new_width}:{new_height},fps=30',  # 固定30帧
            '-c:v', 'libx265',  # 修改为H.265
            '-crf', '20',
            '-preset', 'fast',
            '-tag:v', 'hvc1',
            '-c:a', 'aac',
